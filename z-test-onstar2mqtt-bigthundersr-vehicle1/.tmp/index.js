...
const mqttConfig = {
    ...
    pollingStatusTopic: process.env.MQTT_POLLING_STATUS_TOPIC,
    //commandStatusTopic: process.env.MQTT_COMMAND_STATUS_TOPIC,
};
...
const configureMQTT = async (commands, client, mqttHA) => {
    ...
    client.on('message', (topic, message) => {
        ...
        //const commandString = JSON.parse({command}.command);
        const topicArray = [_.concat({topic},'/',{ command }.command,'/','state')];
        logger.info(topicArray);        
...
(async () => {
    try {
        ...
        const run = async () => {
            ...
            client.publish(mqttConfig.pollingStatusTopic, JSON.stringify({"ok":{"message":"Data Polled Successfully"}}), {retain: false})
        };
        ...
        const main = async () => run()
            ...
            .catch((e) => {
                if (e instanceof Error) {
                  ...
                  client.publish(mqttConfig.pollingStatusTopic, errorJson, {retain: false});
